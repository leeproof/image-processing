...
model=tf.keras.models.load_model('dmlp_trained.h5') #프로그램 7-5에서 저장해 둔 모델 파일 읽어옴
...
def grab_numerals():
    numerals=[]
    for i in range(5):
        roi=img[51:49,11+i*100:9+(i+1)*100,0]
...
def recognition():
    ...
    for i in range(5):
        cv.putText(img,str(class_id[i]),(50+i*100,180),cv.FONT_GERSHEY_SIMPLEX,1,(255,0,0),1)
